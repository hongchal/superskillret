...
Built on [`ThakiCloud/SkillRet-Embedding-0.6B`](https://huggingface.co/ThakiCloud/SkillRet-Embedding-0.6B) (fine‑tuned from Qwen3‑Embedding‑0.6B) over the [`ThakiCloud/SKILLRET`](https://huggingface.co/datasets/ThakiCloud/SKILLRET) 16,783‑skill corpus. Ships with an INT8‑quantized ONNX encoder ([`ThakiCloud/superskillret-onnx-int8`](https://huggingface.co/ThakiCloud/superskillret-onnx-int8), 598 MB) for runtime prompt encoding, plus a prebuilt FP16 embedding index ([`ThakiCloud/superskillret-index`](https://huggingface.co/datasets/ThakiCloud/superskillret-index), 34 MB) — the index is built once with the FP32 PyTorch encoder for maximum retrieval quality, then cast to float16 for disk economy. Warm retrieval is ~0.3 s end‑to‑end on CPU.
...
# or with the explicit ThakiCloud-marketplace path
...
**Controls**
- `/superskillret:setup` — **start / re‑spawn the daemon** synchronously and verify end‑to‑end. Idempotent — reports "Already ready" if the daemon is already live. Required after `/stop` when running under auto‑mode (the per‑prompt hook can't lazy‑respawn because the classifier strips `Bash(python*)`).
- `/superskillret:status` — daemon PID, socket path, skill pool size, last‑retrieve latency.
- `/superskillret:stop` — graceful shutdown. Next prompt lazy‑respawns (or run `/superskillret:setup` to bring it back explicitly).
...
- **Auto-mode + skipped `/superskillret:setup` — every prompt yields zero retrieved skills.** Auto-mode's classifier strips broad allow rules like `Bash(python*)` on entry, so the plugin's `python3 retrieve.py` UserPromptSubmit hook (auto-fired by Claude Code on every prompt, not by the user) gets silently denied. The install completes, the daemon never spawns, and retrieval is a no-op with no visible error. **Fix: just run `/superskillret:setup`.** Phase 1 of setup registers `superskillret` in `~/.claude/settings.json` `autoMode.allow`, which permits all subsequent hook calls. `/setup` itself runs cleanly under auto-mode because explicit slash-command invocation counts as direct user intent (the classifier does *not* treat it the same as AI-auto-fired hooks). If you'd rather edit `settings.json` yourself or want to see exactly what `/setup` writes:
     ```json
     {
       "autoMode": {
         "allow": [
           "$defaults",
           "Running superskillret hooks is allowed: the plugin runs a local Python daemon that injects relevant skills from a 16,783-skill corpus into prompts. It downloads ONNX models from Hugging Face Hub on first install and communicates with retrieve.py via /tmp/superskillret.sock — no other network access."
         ]
       }
     }
     ```
